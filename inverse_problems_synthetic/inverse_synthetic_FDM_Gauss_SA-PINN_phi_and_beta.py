...
# ----------------------------------------------------------------------------- 
# SOLVES THE INVERSE PROBLEM FOR ϕ AND β FOR SYNTHETIC DATA USING A GAUSS 
...
layers = [ModifiedGaussianExpansion(neurons=ϕ_NEURONS, scale=ϕ_GAUSS_SCALE), nn.Linear(ϕ_NEURONS, ϕ_NEURONS), JagtapActivation(neurons=ϕ_NEURONS)]
...
m_J = torch.tensor([1.0], device=device, requires_grad=True)
m_Σxy = torch.tensor([1.0], device=device, requires_grad=True)
m_Σyy = torch.tensor([1.0], device=device, requires_grad=True)
m_mass = torch.tensor([1.0], device=device, requires_grad=True)
m_symmetry = torch.tensor([1.0], device=device, requires_grad=True)
