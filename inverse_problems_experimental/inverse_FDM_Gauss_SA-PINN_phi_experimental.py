...
# ----------------------------------------------------------------------------- 
# SOLVES THE INVERSE PROBLEM FOR ϕ FOR EXPERIMENTAL DATA USING A GAUSS 
...
p = torch.tensor(18000, dtype=torch.float32, device=device)  # steady state pressure (Pa)
Ux_max = torch.tensor(df['U_0'].values.max(), dtype=torch.float32, device=device)  # max steady state velocity (m/s)
ϕ_average = torch.tensor(0.15, dtype=torch.float32, device=device)  # average ϕ (dimensionless)
H = torch.tensor(50e-6, dtype=torch.float32, device=device)  # channel height (m)
...
β = torch.tensor(1.2, dtype=torch.float32, device=device)  # power-law coefficient (or nn.Parameter if learned)
...
non_zero_parameter = nn.Parameter(torch.tensor([0.1], dtype=torch.float32, device=device))
...
y_uniform = (torch.linspace(-1.0, 1.0, COLLOCATION_PTS + 2, device=device) ** 3).unsqueeze(1).requires_grad_(True)[1:-1]

# Trial functions
Ux_trial = lambda y: Ux_PINN(torch.cat([y.to(device)], dim=1))[:,0:1]  # torch.Size([y, 1]), normalized 
ϕ_trial = lambda y: ϕ_max * torch.sigmoid(ϕ_PINN(y.to(device))[:,0:1]) * (1 + y.to(device)) * (1 - y.to(device)) + torch.abs(non_zero_parameter)  # torch.Size([y, 1]), sigmoid keeps bounded between 0 and ϕ_max
...
def physics_loss():  # physics ensures ∇⋅J = ∇⋅Σ = 0
    ...
    dUxstar_dystar_center = (Uxstar[2:] - Uxstar[:-2]) / (2 * Δystar)  # central difference everywhere except at walls 
    dUxstar_dystar_left = (Uxstar[1] - Uxstar[0]) / Δystar  # forward difference at left wall
    dUxstar_dystar_right = (Uxstar[-1] - Uxstar[-2]) / Δystar  # backward difference at right wall
    dUxstar_dystar = torch.cat([dUxstar_dystar_left.unsqueeze(1), dUxstar_dystar_center, dUxstar_dystar_right.unsqueeze(1)], dim=0)  # torch.Size([N, 1])
    ...
    dΣpxystar_dystar_center = ((Σpstar[:, 0, 1][2:] - Σpstar[:, 0, 1][:-2]) / (2 * Δystar)).unsqueeze(1)  # central difference everywhere except at walls 
    dΣpxystar_dystar_left = ((Σpstar[:, 0, 1][1] - Σpstar[:, 0, 1][0]) / Δystar).unsqueeze(0).unsqueeze(0)  # forward difference at left wall
    dΣpxystar_dystar_right = ((Σpstar[:, 0, 1][-1] - Σpstar[:, 0, 1][-2]) / Δystar).unsqueeze(0).unsqueeze(0)  # backward difference at right wall
    dΣpxystar_dystar = torch.cat([dΣpxystar_dystar_left, dΣpxystar_dystar_center, dΣpxystar_dystar_right], dim=0)  # torch.Size([N, 1])

    dΣpyystar_dystar_center = ((Σpstar[:, 1, 1][2:] - Σpstar[:, 1, 1][:-2]) / (2 * Δystar)).unsqueeze(1)  # central difference everywhere except at walls 
    dΣpyystar_dystar_left = ((Σpstar[:, 1, 1][1] - Σpstar[:, 1, 1][0]) / Δystar).unsqueeze(0).unsqueeze(0)  # forward difference at left wall
    dΣpyystar_dystar_right = ((Σpstar[:, 1, 1][-1] - Σpstar[:, 1, 1][-2]) / Δystar).unsqueeze(0).unsqueeze(0)  # backward difference at right wall
    dΣpyystar_dystar = torch.cat([dΣpyystar_dystar_left, dΣpyystar_dystar_center, dΣpyystar_dystar_right], dim=0)  # torch.Size([N, 1])
    ...
    dJystar_dystar_center = ((Jstar[:, 1, 0][2:] - Jstar[:, 1, 0][:-2]) / (2 * Δystar)).unsqueeze(1)  # central difference everywhere except at walls 
    dJystar_dystar_left = ((Jstar[:, 1, 0][1] - Jstar[:, 1, 0][0]) / Δystar).unsqueeze(0).unsqueeze(0)  # forward difference at left wall
    dJystar_dystar_right = ((Jstar[:, 1, 0][-1] - Jstar[:, 1, 0][-2]) / Δystar).unsqueeze(0).unsqueeze(0)  # backward difference at right wall
    dJystar_dystar = torch.cat([dJystar_dystar_left, dJystar_dystar_center, dJystar_dystar_right], dim=0)  # torch.Size([N, 1])
    ...
    dΣxystar_dystar_center = ((Σstar[:, 0, 1][2:] - Σstar[:, 0, 1][:-2]) / (2 * Δystar)).unsqueeze(1)  # central difference everywhere except at walls 
    dΣxystar_dystar_left = ((Σstar[:, 0, 1][1] - Σstar[:, 0, 1][0]) / Δystar).unsqueeze(0).unsqueeze(0)  # forward difference at left wall
    dΣxystar_dystar_right = ((Σstar[:, 0, 1][-1] - Σstar[:, 0, 1][-2]) / Δystar).unsqueeze(0).unsqueeze(0)  # backward difference at right wall
    dΣxystar_dystar = torch.cat([dΣxystar_dystar_left, dΣxystar_dystar_center, dΣxystar_dystar_right], dim=0) - dpstar_dxstar  # torch.Size([N, 1])

    dΣyystar_dystar_center = ((Σstar[:, 1, 1][2:] - Σstar[:, 1, 1][:-2]) / (2 * Δystar)).unsqueeze(1)  # central difference everywhere except at walls 
    dΣyystar_dystar_left = ((Σstar[:, 1, 1][1] - Σstar[:, 1, 1][0]) / Δystar).unsqueeze(0).unsqueeze(0)  # forward difference at left wall
    dΣyystar_dystar_right = ((Σstar[:, 1, 1][-1] - Σstar[:, 1, 1][-2]) / Δystar).unsqueeze(0).unsqueeze(0)  # backward difference at right wall
...
# Training Loop ---------------------------------------------------------------
# Define parameters
ϕ_PINN_parameters = list(ϕ_PINN.parameters()) + [non_zero_parameter]  # PINN parameters (include all)
