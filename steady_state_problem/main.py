...
inverse_problem = c.inverse_problem
...
def total_loss(J_global, Σxy_global, Σyy_global, mass_global, symmetry_global, data_global):  # combining losses
    ...
    # Do not call physics_loss() if only training u 
        ...
        # ---------------------------------------------------------------
            ...
            if u_epochs_ADAM != 0:
                for epoch in range(u_epochs_ADAM + 1):
                    ...
                    ℒ, ℒ_visualize, ℒ_individuals, ℒ_individuals_visualize, lift_force_visualize = total_loss(J_global=0, Σxy_global=0, Σyy_global=0, mass_global=0, symmetry_global=0, data_global=1 if inverse_problem else 0)
            ...
            if u_epochs_LBFGS != 0:
                def closure_u():
                    ...
                    ℒ, _, _, _, _ = total_loss(J_global=0, Σxy_global=0, Σyy_global=0, mass_global=0, symmetry_global=0, data_global=1 if inverse_problem else 0)
                    ℒ.backward()
                    return ℒ
    ...
    # ---------------------------------------------------------------
        ...
        if (u_epochs_ADAM or ϕ_epochs_ADAM) != 0:
            ...
            for epoch in range(most_epochs_ADAM):
                ...
                # Forward & Backward passes
                ℒ, ℒ_visualize, ℒ_individuals, ℒ_individuals_visualize, lift_force_visualize = total_loss(J_global=1, Σxy_global=1, Σyy_global=1, mass_global=1, symmetry_global=1, data_global=1 if inverse_problem else 0)
                ℒ.backward()
        ...
        if (u_epochs_LBFGS or ϕ_epochs_LBFGS) != 0:
            def closure_joint():
                ...
                ℒ, _, _, _, _ = total_loss(J_global=1, Σxy_global=1, Σyy_global=1, mass_global=1, symmetry_global=1, data_global=1 if inverse_problem else 0)
                ℒ.backward()
                return ℒ
