...
data_file_directory = 'synthetic_data_example_2'
# default options:
...
# Performance
use_GPU: bool = False  # if True, uses GPU if available
...
scheduler_factor: float = 0.7  # factor by which the learning rate will be reduced
scheduler_min_learning_rate: float = 1e-6  # lower bound on the learning rate
...
# Inverse vs. forward problem 
inverse_problem: bool = True  # if False, the forward problem is solved with known β, but no know velocity data
# NOTE: the 'forward problem' simply omits the use of velocity data during training
...
u_PINN_Fourier_scale: float = 0  # only for two_PINNs = True | scale for Fourier feature mapping
...
u_PINN_epochs_ADAM: int = 1000
u_PINN_epochs_LBFGS: int = 0
u_PINN_learning_rate: float = 0.01
...
# Lift force 
β_learnable: bool = True  # if True, β is learned during training; if False, β is fixed to 'beta' from the parameters.yaml file
β_initial_guess: float = 1
β_learning_rate: float = 0.001

# Spatially-adaptive learnable parameters
use_spatially_adaptive_learnable_parameters: bool = True  
