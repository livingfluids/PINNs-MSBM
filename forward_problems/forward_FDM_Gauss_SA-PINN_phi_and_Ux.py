...
# ----------------------------------------------------------------------------- 
# SOLVES THE FORWARD PROBLEM FOR ϕ AND Ux USING A GAUSS EXPANSION LAYER AND 
# THE FINITE DIFFERENCE METHOD FOR CALCULATING DERIVATIVES IN THE PDE LOSS
# ----------------------------------------------------------------------------- 
...
use_scheduler = False  # if True, the learning rates will decrease over time as the loss plateaus
...
β = torch.tensor(1.2 if data_file_1 else 1.1, dtype=torch.float32, device=device)  # power-law coefficient (or nn.Parameter if learned)
frv = torch.tensor(1.2, dtype=torch.float32, device=device)  # function of the reduced volume
dpstar_dxstar = torch.tensor([-2.4855 if data_file_1 else -3.2355], dtype=torch.float32, device=device) # normalized x-pressure gradient (dimensionless)
...
# 'Jagtap' activation function from Jagtap et al. does not make a noticable difference
...
def physics_loss():  # physics ensures ∇⋅J = ∇⋅Σ = 0
    ...
    dUxstar_dystar_center = (Uxstar[2:] - Uxstar[:-2]) / (2 * Δystar)
    dUxstar_dystar_left = (Uxstar[1] - Uxstar[0]) / Δystar 
    dUxstar_dystar_right = (Uxstar[-1] - Uxstar[-2]) / Δystar
    ...
    dΣpxystar_dystar_center = ((Σpstar[:, 0, 1][2:] - Σpstar[:, 0, 1][:-2]) / (2 * Δystar)).unsqueeze(1)
    dΣpxystar_dystar_left = ((Σpstar[:, 0, 1][1] - Σpstar[:, 0, 1][0]) / Δystar).unsqueeze(0).unsqueeze(0)
    dΣpxystar_dystar_right = ((Σpstar[:, 0, 1][-1] - Σpstar[:, 0, 1][-2]) / Δystar).unsqueeze(0).unsqueeze(0)
    dΣpxystar_dystar = torch.cat([dΣpxystar_dystar_left, dΣpxystar_dystar_center, dΣpxystar_dystar_right], dim=0)  # torch.Size([N, 1])

    dΣpyystar_dystar_center = ((Σpstar[:, 1, 1][2:] - Σpstar[:, 1, 1][:-2]) / (2 * Δystar)).unsqueeze(1)
    dΣpyystar_dystar_left = ((Σpstar[:, 1, 1][1] - Σpstar[:, 1, 1][0]) / Δystar).unsqueeze(0).unsqueeze(0)
    dΣpyystar_dystar_right = ((Σpstar[:, 1, 1][-1] - Σpstar[:, 1, 1][-2]) / Δystar).unsqueeze(0).unsqueeze(0)
    ...
    dJystar_dystar_center = ((Jstar[:, 1, 0][2:] - Jstar[:, 1, 0][:-2]) / (2 * Δystar)).unsqueeze(1)
    dJystar_dystar_left = ((Jstar[:, 1, 0][1] - Jstar[:, 1, 0][0]) / Δystar).unsqueeze(0).unsqueeze(0)
    dJystar_dystar_right = ((Jstar[:, 1, 0][-1] - Jstar[:, 1, 0][-2]) / Δystar).unsqueeze(0).unsqueeze(0)
    ...
    dΣxystar_dystar_center = ((Σstar[:, 0, 1][2:] - Σstar[:, 0, 1][:-2]) / (2 * Δystar)).unsqueeze(1)
    dΣxystar_dystar_left = ((Σstar[:, 0, 1][1] - Σstar[:, 0, 1][0]) / Δystar).unsqueeze(0).unsqueeze(0)
    dΣxystar_dystar_right = ((Σstar[:, 0, 1][-1] - Σstar[:, 0, 1][-2]) / Δystar).unsqueeze(0).unsqueeze(0)
    ...
    dΣyystar_dystar_center = ((Σstar[:, 1, 1][2:] - Σstar[:, 1, 1][:-2]) / (2 * Δystar)).unsqueeze(1)
    dΣyystar_dystar_left = ((Σstar[:, 1, 1][1] - Σstar[:, 1, 1][0]) / Δystar).unsqueeze(0).unsqueeze(0)
...
ϕ_PINN_parameters = list(ϕ_PINN.parameters())
λ_parameters = [λ_J, λ_Σxy, λ_Σyy, λ_mass, λ_symmetry]  # self-adaptive weights (include all)

# Define optimizers
Ux_PINN_optimizer_Adam = torch.optim.Adam(Ux_PINN_parameters, lr=Ux_LEARNING_RATE)
ϕ_PINN_optimizer_Adam = torch.optim.Adam(ϕ_PINN_parameters, lr=ϕ_LEARNING_RATE)
ϕ_PINN_optimizer_LBFGS = torch.optim.LBFGS(list(ϕ_PINN.parameters()), lr=ϕ_LEARNING_RATE)
λ_optimizer = torch.optim.Adam(λ_parameters, lr=λ_LEARNING_RATE)

# Define schedulers (ReduceLROnPlateau type does exactly that)
Ux_PINN_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(Ux_PINN_optimizer_Adam, factor=SCHEDULER_FACTOR, patience=SCHEDULER_PATIENCE, min_lr=SCHEDULER_MIN_LR)
ϕ_PINN_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(ϕ_PINN_optimizer_Adam, factor=SCHEDULER_FACTOR, patience=SCHEDULER_PATIENCE, min_lr=SCHEDULER_MIN_LR)
...
for epoch in range(EPOCHS_ADAM):
    ...
    Ux_PINN_scheduler.step(ℒ_un.item()) if use_scheduler else None
    ϕ_PINN_optimizer_Adam.step()  # gradient descent updating PINN parameters
    ϕ_PINN_scheduler.step(ℒ_un.item()) if use_scheduler else None
